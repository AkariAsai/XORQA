...
class ReaderYesNo(nn.Module):

    def __init__(self, encoder: nn.Module, hidden_size, n_class):
    ...
    def forward(self, input_ids: T, attention_mask: T, start_positions=None, end_positions=None, answer_mask=None, switch_labels=None):
        # notations: N - number of questions in a batch, M - number of passages per questions, L - sequence length
        N, M, L = input_ids.size()
        start_logits, end_logits, relevance_logits = self._forward(input_ids.view(N * M, L),
                                                                   attention_mask.view(N * M, L))
...
def compute_loss_yes_no(start_positions, end_positions, switch_labels, answer_mask, start_logits, end_logits, relevance_logits, N, M):
    start_positions = start_positions.view(N * M, -1)
    end_positions = end_positions.view(N * M, -1)
    answer_mask = answer_mask.view(N * M, -1)
    ...
    switch_losses = [(loss_fct(relevance_logits, _switch) * _answer_mask) \
                    for (_switch, _answer_mask) \
                    in zip(torch.unbind(switch_labels, dim=1), torch.unbind(answer_mask, dim=1))]
...
def create_reader_input_yes_no(pad_token_id: int,
                        samples: List[ReaderSample],
                        passages_per_question: int,
                        max_length: int,
                        max_n_answers: int,
...
def _get_positive_idx_has_answer(positives: List[ReaderPassage], is_random: bool):
    # select just one positive
    positive_idx = np.random.choice(len(positives)) if is_random else 0
    return positive_idx
...
# FIXME: add codes for yes/no
def _create_question_passages_tensors_yes_no(positives: List[ReaderPassage], negatives: List[ReaderPassage], total_size: int,
                                      empty_ids: T,
                                      max_n_answers: int,
                                      pad_token_id: int,
                                      is_train: bool,
